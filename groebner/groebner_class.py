...
class Groebner(object):
    ...
    def initialize_np_matrix(self, final_time = False):
        ...
        if final_time:
            ...
            for poly in polys:
                lms[poly.lead_term].append(poly)

            polys_with_unique_lms = list()

            for i in lms:
                if len(lms[i]) > 1: #It's duplicated
                    #self.duplicate_lms.add(Term(i))
                    self.new_polys.append(lms[i][0])
                    polys_with_unique_lms.append(lms[i][0])
            ...
            for i in polys_with_unique_lms:
                for j in polys_with_unique_lms:
                    if i != j:
